...
def returnCar():
    ...
    def carUpdate():
        ...
        submit_cur.execute("SELECT Name, Case When R.PaymentDate = 'NULL' Then R.TotalAmount When R.PaymentDate <> 'NULL' Then 0 End Balance, R.VehicleID, C.CustID From CUSTOMER AS C, RENTAL AS R, Vehicle AS V Where C.CustId = R.CustID AND V.VehicleID = R.VehicleID AND C.Name = \'"+name1+"\' and R.ReturnDate = \'"+date+"\' and V.Description = \'"+des1+"\'")
        ...
        newRes = printRes.split()
        newPrint = ''

        for i in range(len(newRes)-2):
            newPrint += newRes[i] + " "
        


        RentalCarId = newRes[len(newRes)-2]
        RentalCustId = newRes[len(newRes)-1]
        print(RentalCarId)
        print(RentalCustId)

        title_label = Label(wind,text="Username & Balance Left")
        title_label.grid(row=5,column=1)
        res_label = Label(wind,text=newPrint)
        res_label.grid(row=6, column=1)
        
       

        submit_cur.execute("UPDATE RENTAL set PaymentDate =ReturnDate WHERE CustID = \'"+RentalCustId+"\' and ReturnDate = \'"+date+"\' and VehicleID = \'"+RentalCarId+"\'")
    ...
    ret_label = Label(wind,text="Return Date: ")
    ret_label.grid(row=1,column=0)
    retDate = Entry(wind,width="30")
    retDate.grid(row=1,column=1,padx=20)

    des_label = Label(wind,text="Return Date: ")
    des_label.grid(row=2,column=0)
    des = Entry(wind,width="30")
    des.grid(row=2,column=1,padx=20)

    newCus = Button(wind, text="Update", command=carUpdate)
    newCus.grid(row=3, column=1, padx=20)
...
carReturn = Button(root, text="Handle Return of Rented Car", command=returnCar)
carReturn.pack()
